...
namespace {
    ...
    [[nodiscard]] consteval cc::CArray<uint8_t, 256> make_ident_6bit_lut() noexcept {
        cc::CArray<uint8_t, 256> lut{};
        for (int i = 0; i < 256; ++i) {
...
constexpr alignas(64) cc::CArray<uint8_t, 256> IDENT_CHAR_LUT = make_ident_6bit_lut();

[[nodiscard]] inline constexpr bool IS_IDENT_CHAR(char c) noexcept {
    return IDENT_CHAR_LUT[static_cast<unsigned char>(c)] != 0;
}

using CharBitLUT = cc::CArray<std::uint64_t, 4>;

namespace {
    [[nodiscard]] consteval cc::CArray<std::uint64_t, 4> make_ident_bitmask() noexcept {
        cc::CArray<std::uint64_t, 4> mask{};
...
constexpr alignas(64) cc::CArray<uint64_t, 4> IDENT_CHAR_BITMASK = make_ident_bitmask();
